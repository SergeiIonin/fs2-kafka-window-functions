...
class CollectorServiceTimeWindowsImpl[F[_] : Async, K, V](stateRef: Ref[F, Chunk[CommittableConsumerRecord[F, K, V]]],
                                                          timeRef: Ref[F, Long],
                                                          durationMillis: Long,
                                                          releaseChunk: Chunk[CommittableConsumerRecord[F, K, V]] => F[Unit])
                                                         (implicit logger: Logger) extends StreamCollectorService[F, CommittableConsumerRecord[F, K, V]] {
  ...
  override def addToChunk(cr: CommittableConsumerRecord[F, K, V]): F[Unit] = {
    timeRef.update(time => {
      if (time == 0L) cr.record.timestamp.createTime.getOrElse(System.currentTimeMillis()) else time
    }) >> super.addToChunk(cr)
  }
...
object CollectorServiceTimeWindowsImpl {
  ...
  def make[F[_] : Async, K, V](durationMillis: Long,
