...
class CollectorServiceTimeWindowsImpl[F[_] : Async, K, V](stateRef: Ref[F, Chunk[ChunkedRecord[F, K, V]]],
                                                                    timeRef: Ref[F, Long],
                                                          durationMillis: Long,
                                                          releaseChunk: Chunk[ChunkedRecord[F, K, V]] => F[Unit]) extends CollectorService[F, K, V] {
  ...
  private val resetStartTime: F[Unit] = timeRef.update(_ => System.currentTimeMillis()).void
