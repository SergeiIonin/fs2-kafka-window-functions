...
object StreamTimeWindowAggregatorService {
  ...
  def clearingStreamResource[F[_] : Async, R](durationMillis: Long, chunksRef: Ref[F, Map[Long, Chunk[R]]],
                                              onRelease: Chunk[R] => F[Unit])(implicit logger: Logger) =
  Resource.make(fs2.Stream.awakeEvery(FiniteDuration(durationMillis * 10, MILLISECONDS))
      .evalMap(_ =>
