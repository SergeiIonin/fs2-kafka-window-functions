...
class KafkaConnSpec extends AnyFlatSpec with Matchers with LogSupport {
  ...
  "Chunking Service" should "aggregate records within a timeframe and count the number of records within each frame" in {
    ...
    } yield (consumerService, chunkingService))
      .use { case (consumer, chunkingService) =>
    ...
    val list =
      buf.toList
        .sortBy { case (k, _) => k }
        .map { case (_, chunk) => chunk.map(_.record.timestamp.createTime.get) }
        .zipWithIndex

    println(s"buf = $buf")

    println(s"list = ${list.map { case (chunk, ind) => ind -> chunk.toList.mkString(", ") }.mkString(", \n")}")

    list.foreach { case (recKeys, index) => recKeys shouldBe (index to index * 10).toList.map(i => s"key-$i") }
