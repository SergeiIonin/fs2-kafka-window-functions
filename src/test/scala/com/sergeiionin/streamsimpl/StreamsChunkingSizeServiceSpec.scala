...
//
    ...
    def emit(createTime: Long): fs2.Stream[IO, TestRecord] = fs2.Stream
    ...
    list.foreach { case (_, list) => list.sorted shouldBe list }
...
object StreamsChunkingSizeServiceSpec {
  ...
  class StreamCollectorServiceChunkSizeImpl[F[_]: Async](
    ...
    override val addCond: TestRecord => F[Boolean] = _ => stateRef.modify(chunk => chunk -> (chunk.size < size))

    override val releaseCond: Chunk[TestRecord] => F[Boolean] = chunk => (chunk.size >= size).pure[F]

    override val onRelease: Chunk[TestRecord] => F[Unit] = releaseChunk
  }
  ...
  object StreamCollectorServiceChunkSizeImpl {
    def make[F[_]: Async](
      ...
      logger:    Logger
    ): Resource[F, StreamCollectorServiceChunkSizeImpl[F]] = Resource.eval(
      Async[F]
        .ref(Chunk.empty[TestRecord])
        .map(stateRef => new StreamCollectorServiceChunkSizeImpl(stateRef, size, onRelease) {})
    )
  }
